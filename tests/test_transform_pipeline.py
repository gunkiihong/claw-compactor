"""Tests for Transform Pipeline: base classes, CompressContext, TransformResult, and Pipeline engine."""
...
from lib.transforms.pipeline import Pipeline, PipelineResult, StepResult


# ---------------------------------------------------------------------------
# Concrete Transform helpers
# ---------------------------------------------------------------------------
...
class AppendTransform(Transform):
...
class NeverApplyTransform(Transform):
    """Never applies."""
    name = "never"
    order = 5
...
class HighOrderTransform(Transform):
    ...
    def apply(self, ctx: CompressContext) -> TransformResult:
        ...
        return TransformResult(content=result, original_tokens=1, compressed_tokens=1)


class LowOrderTransform(Transform):
...
# ---------------------------------------------------------------------------
# CompressContext tests
# ---------------------------------------------------------------------------

class TestCompressContext:
    def test_defaults(self):
        ctx = CompressContext(content="hello")
    ...
    def test_evolve_returns_new_instance(self):
        ctx = CompressContext(content="hello", content_type="text")
    ...
    def test_evolve_original_unchanged(self):
        ctx = CompressContext(content="original", content_type="text")
        ctx.evolve(content="changed")
        assert ctx.content == "original"

    def test_evolve_only_changes_specified_fields(self):
        ctx = CompressContext(content="hello", content_type="code", role="system", language="python")
        evolved = ctx.evolve(content="world")
    ...
    def test_metadata_default_is_empty_dict(self):
        ctx1 = CompressContext(content="a")
        ctx2 = CompressContext(content="b")
...
class TestTransformResult:
    def test_defaults(self):
        result = TransformResult(content="hello")
    ...
    def test_skipped_flag_default_false(self):
        result = TransformResult(content="x")
        assert result.skipped is False

    def test_lists_are_independent_per_instance(self):
        r1 = TransformResult(content="a")
        r2 = TransformResult(content="b")
...
class TestTimedApply:
    ...
    def test_skipped_result_preserves_original_content(self):
        transform = NeverApplyTransform()
        ctx = CompressContext(content="preserve me", content_type="text")
        result = transform.timed_apply(ctx)
    ...
    def test_timing_is_nonzero_for_slow_transform(self):
        class SlowTransform(Transform):
            ...
            def apply(self, ctx):
                time.sleep(0.02)
                return TransformResult(content=ctx.content, original_tokens=1, compressed_tokens=1)
    ...
    def test_applies_transform_when_should_apply_true(self):
        ...
        ctx = CompressContext(content="hello world", content_type="text")
        result = transform.timed_apply(ctx)
        assert result.skipped is False
    ...
    def test_timed_apply_copies_markers_and_warnings(self):
        ...
        ctx = CompressContext(content="test")
        result = transform.timed_apply(ctx)
    ...
    def test_upper_does_not_apply_to_non_text(self):
        transform = UpperTransform()
        ctx = CompressContext(content="hello world", content_type="code")
        result = transform.timed_apply(ctx)
        assert result.skipped is True
        assert result.content == "hello world"
...
class TestPipeline:
    def test_empty_pipeline_returns_original_content(self):
        pipeline = Pipeline()
        ctx = CompressContext(content="hello world")
        pr = pipeline.run(ctx)
    ...
    def test_single_transform_applied(self):
        pipeline = Pipeline([UpperTransform()])
        ctx = CompressContext(content="hello world", content_type="text")
        pr = pipeline.run(ctx)
        assert pr.content == "HELLO WORLD"
    ...
    def test_runs_transforms_in_order_by_order_field(self):
        ...
        pipeline = Pipeline([HighOrderTransform(), LowOrderTransform()])
        ctx = CompressContext(content="x")
        pr = pipeline.run(ctx)
    ...
    def test_chains_output_each_transform_receives_previous_output(self):
        ...
        pipeline = Pipeline([tracker, upper])
        ctx = CompressContext(content="hello", content_type="text")
        pr = pipeline.run(ctx)
    ...
    def test_collects_markers_from_all_steps(self):
        class MarkerA(Transform):
            ...
            def apply(self, ctx):
                return TransformResult(content=ctx.content, original_tokens=1, compressed_tokens=1, markers=["marker-a"])

        class MarkerB(Transform):
            name = "mb"
            order = 2
            ...
            def apply(self, ctx):
                return TransformResult(content=ctx.content, original_tokens=1, compressed_tokens=1, markers=["marker-b"])

        pipeline = Pipeline([MarkerA(), MarkerB()])
        ctx = CompressContext(content="x")
        pr = pipeline.run(ctx)
    ...
    def test_collects_warnings_from_all_steps(self):
        class WarnA(Transform):
            ...
            def apply(self, ctx):
                return TransformResult(content=ctx.content, original_tokens=1, compressed_tokens=1, warnings=["warn-a"])

        class WarnB(Transform):
            name = "wb"
            order = 2
            ...
            def apply(self, ctx):
                return TransformResult(content=ctx.content, original_tokens=1, compressed_tokens=1, warnings=["warn-b"])

        pipeline = Pipeline([WarnA(), WarnB()])
        ctx = CompressContext(content="x")
        pr = pipeline.run(ctx)
    ...
    def test_skipped_transform_does_not_contribute_markers(self):
        class SkippedWithMarkers(Transform):
            name = "skip_marker"
            order = 1
            ...
            def apply(self, ctx):
                return TransformResult(content=ctx.content, original_tokens=1, compressed_tokens=1, markers=["should-not-appear"])

        pipeline = Pipeline([SkippedWithMarkers()])
        ctx = CompressContext(content="x")
        pr = pipeline.run(ctx)
        assert "should-not-appear" not in pr.markers

    def test_skipped_transform_does_not_change_content(self):
        pipeline = Pipeline([NeverApplyTransform()])
        ctx = CompressContext(content="unchanged")
        pr = pipeline.run(ctx)
        assert pr.content == "unchanged"

    def test_total_timing_accumulated(self):
        pipeline = Pipeline([AppendTransform(" a"), AppendTransform(" b")])
        ctx = CompressContext(content="x")
        pr = pipeline.run(ctx)
    ...
    def test_steps_list_contains_all_transform_names(self):
        pipeline = Pipeline([UpperTransform(), AppendTransform()])
        ctx = CompressContext(content="hello", content_type="text")
        pr = pipeline.run(ctx)
    ...
    def test_empty_content_handled(self):
        pipeline = Pipeline([UpperTransform()])
        ctx = CompressContext(content="", content_type="text")
        pr = pipeline.run(ctx)
        assert pr.content == ""

    def test_pipeline_result_is_pipeline_result_type(self):
        pipeline = Pipeline()
        ctx = CompressContext(content="hello")
        pr = pipeline.run(ctx)
    ...
    def test_multiple_transforms_chain_content(self):
        ...
        pipeline = Pipeline([AppendTransform(" [done]"), UpperTransform()])
        ctx = CompressContext(content="hello", content_type="text")
        pr = pipeline.run(ctx)
    ...
    def test_pipeline_with_all_skipped_returns_original(self):
        pipeline = Pipeline([NeverApplyTransform()])
        ctx = CompressContext(content="original text")
        pr = pipeline.run(ctx)
